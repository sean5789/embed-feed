    ...
    const firstBatch = postsForClient.slice(0, BATCH_SIZE).map(post => `
      <div class="card">
        ...
        <div class="info">
          ...
          <div class="tag">
            <a href="${CAL_URL}" target="_blank" rel="noopener noreferrer">Discover !</a>
          </div>
        </div>
      </div>
    `).join("\n");
...
<head>
  ...
  <style>
    ...
    .date { font-size:15px; color:#000; font-weight:bold; margin-top: 13px;}
    .tag { margin-top:1px; display:inline-block; }
    .tag a {
      color:inherit; text-decoration:none; display:inline-block;
      background:white; font-weight:bold; padding:3px; border-radius:6px; font-size: 16px;
    }
